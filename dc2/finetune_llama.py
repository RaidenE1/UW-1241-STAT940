...
def main():
    ...
    model = AutoModelForSequenceClassification.from_pretrained("./Llama-2-7b-hf", num_labels=3)
    ...
    if args.local_rank == 0:
        # prepare logging
        if args.val:
            model_name = "%s_e%d_b%d_lr%d_%s_token512" % ("llama_val", \
                epochs, batch_size, learning_rate * 10000000, args.optim)
            work_dir = os.path.join("./out", "llama_val", model_name)
        else:
    ...
    if args.local_rank == 0:
        ...
        if args.val:
            df.to_csv('./out/%s/%s/%s.csv' % ("llama_val", model_name, model_name), index=False)
        else:
